...
class FR24Cache:
    ...
    def __init__(self, path: Path | str) -> None:
        ...
        self.live_feed = TimestampedCache(Collection(self.path / "feed"))
        ...
        self.top_flights = TimestampedCache(
            Collection(self.path / "top_flights")
        )
...
class FlightListRegCache(GlobMixin):
    ...
    def scan_table(
        ...
        """
        return scan_table(self.get_path(reg), format=format)
...
class FlightListFlightCache(GlobMixin):
    ...
    def scan_table(
        ...
        """
        return scan_table(self.get_path(flight), format=format)
...
class PlaybackCache(GlobMixin):
    ...
    def scan_table(
        ...
        """
        return scan_table(self.get_path(flight_id), format=format)
...
class TimestampedCache(GlobMixin):
    ...
    def scan_table(
        ...
        *,
        ...
        """
        return scan_table(self.get_path(timestamp), format=format)
...
class NearestFlightsCache(GlobMixin):
    ...
    def get_path(
        self, lat: float, lon: float, timestamp: IntoTimestamp | str
    ) -> BarePath:
        lon6 = int(lon * 1e6)
        lat6 = int(lat * 1e6)
        ts = to_unix_timestamp(timestamp)
    ...
    def scan_table(
        ...
        *,
        ...
        :param timestamp: A timestamp-like object, see
            [fr24.utils.to_unix_timestamp][]. The `now` literal is not allowed.
        """
        return scan_table(self.get_path(lat, lon, timestamp), format=format)
    ...
    def scan_table(
        ...
        *,
        ...
        :param timestamp: A timestamp-like object, see
            [fr24.utils.to_unix_timestamp][]. The `now` literal is not allowed.
        """
        return scan_table(self.get_path(flight_id, timestamp), format=format)
